...
|:-:|---------|--------|:----------:|:----------:|
| 1 | Blank/empty/whitespace + zero/negative counts | Per-column | High | Low |
| 2 | Cardinality classification | Per-column | High | Low |
| 3 | Min/max string *values* | Per-column | Medium | Low |
...
### 1. Blank/empty/whitespace + zero/negative counts
...
1. **Free riders on the single-pass scan** — #1 blank/zero counts, #2 cardinality classification, #3 min/max string values. High/Medium value, near-zero marginal cost once the Phase 3 rewrite has landed.
